...
# 3. Verificar si el archivo build.ctl tiene cambios
echo "3. Verificando si '$FILE_TO_REVERT' ha cambiado..."
if git diff --name-only | grep -q "^$FILE_TO_REVERT$"; then
  ...
  # 4. Restaurar el archivo con el contenido del archivo base
  ...
  # 5. Hacer commit de los cambios restaurados
  echo "5. Haciendo commit de los cambios restaurados en '$FILE_TO_REVERT'..."
  git add $FILE_TO_REVERT
  git commit -m "Restore $FILE_TO_REVERT with content from $BASE_FILE"
  
  # 6. Pushear los cambios a master
  echo "6. Pusheando los cambios a la rama '$BRANCH_MASTER'..."
